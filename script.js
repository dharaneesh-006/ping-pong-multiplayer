...
function joinRoom() {
  ...
  if (localStorage.getItem("joinedRoom") === room) return alert("You already joined this room.");
...
function startInputListener() {
  ...
  game.addEventListener("touchmove", e => {
    ...
    if (playerSide === "top" || playerSide === "bottom") {
      ref.update({ x: Math.max(0, Math.min(rect.width - rect.width * 0.25, x)) });
    } else {
      ref.update({ y: Math.max(0, Math.min(rect.height - rect.height * 0.25, y)) });
    }
  }, { passive: false });
}
  ...
  setInterval(() => {
    ...
    db.ref(`rooms/${room}/players`).once("value").then(snapshot => {
      ...
      for (let side in players) {
        ...
        let hit = false;
        if (side === "bottom" && vy > 0 && y + 24 >= 290 && x >= p.x && x <= p.x + 100)
          hit = true;
        else if (side === "top" && vy < 0 && y <= 10 && x >= p.x && x <= p.x + 100)
          hit = true;
        else if (side === "left" && vx < 0 && x <= 10 && y >= p.y && y <= p.y + 100)
          hit = true;
        else if (side === "right" && vx > 0 && x + 24 >= 290 && y >= p.y && y <= p.y + 100)
          hit = true;
